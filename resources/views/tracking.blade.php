    ...
    <section class="lg:max-w-4xl xs:max-w-9/10 w-full mx-auto flex flex-col justify-center items-center mt-28 mb-20">
        ...
        <form x-data="{ loading: false }" x-on:submit="loading = true" action="{{ route('tracking.result') }}" method="POST" class="lg:max-w-3xl xs:w-full flex flex-col gap-4 justify-center w-full ring ring-apple-400 p-4 rounded-2xl mt-6">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input name="id" id="id" type="text" pattern="[A-Za-z0-9\s]+" oninput="this.value = this.value.replace(/[^a-zA-Z0-9\s]/g, '').toUpperCase()" class=" bg-gray-50 border border-gray-300 font-display text-xs rounded-lg placeholder:text-gray-400 placeholder:text-xs focus:ring-apple-600 focus:border-apple-400 block w-full p-2.5" placeholder="Tuliskan ID Pengajuan" required autocomplete="off"/>
            </div>            
